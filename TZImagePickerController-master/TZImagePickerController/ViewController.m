...
- (void)viewDidLoad {
    ...
    UIButton * addBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 64, 40, 40)];
    addBtn.backgroundColor = [UIColor redColor];
    [addBtn addTarget:self action:@selector(ClckedUpdataImg) forControlEvents:UIControlEventTouchUpInside];
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    ...
    // 1.打印图片名字
    [self printAssetsName:assets and:photos];
}
...
/// 打印图片名字
- (void)printAssetsName:(NSArray *)assets and:(NSArray *)ImgAry{
    NSString *fileName;
...
//        [imageM addObject:image];
        ...
        [[PHImageManager defaultManager] requestImageForAsset:set targetSize:[UIScreen mainScreen].bounds.size contentMode:PHImageContentModeAspectFit options:options resultHandler:^(UIImage *result, NSDictionary *info) {
            //设置图片
           // [imageM addObject:result];
        }];
    }
    
    for (UIImage * img in ImgAry) {
        [imageM addObject:img];
    }
...
- (void)ClckedUpdataImg{
   ...
   // NSString * path = @"http://sbjkzx.com/api/upload/logoUp";
    ...
    if(imageM.count!=0)
    {
        ...
        [manager startMultiPartUploadTaskWithUrl:path
                                     imagesArray:imageM
                               parameterOfimages:@"img_url"
